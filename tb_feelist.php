...
$tb_fee_list = NULL; // Initialize page object first

class ctb_fee_list extends ctb_fee {
	...
	// Page object name
	var $PageObjName = 'tb_fee_list';

	// Grid form hidden field names
	var $FormName = 'ftb_feelist';
	...
	function __construct() {
		...
		$this->MultiDeleteUrl = "tb_feedelete.php";
		$this->MultiUpdateUrl = "tb_feeupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption ftb_feelistsrch";
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server") {
			$sSavedFilterList = $UserProfile->GetSearchFilters(CurrentUserName(), "ftb_feelistsrch");
		} else {
			$sSavedFilterList = "";
		}
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"ftb_feelistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"ftb_feelistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.ftb_feelist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"ftb_feelistsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_tb_fee\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_tb_fee',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.ftb_feelist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($tb_fee_list)) $tb_fee_list = new ctb_fee_list();

// Page init
$tb_fee_list->Page_Init();

// Page main
$tb_fee_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tb_fee_list->Page_Render();
?>
...
var CurrentForm = ftb_feelist = new ew_Form("ftb_feelist", "list");
ftb_feelist.FormKeyCountName = '<?php echo $tb_fee_list->FormKeyCountName ?>';

// Form_CustomValidate event
ftb_feelist.Form_CustomValidate = 
...
ftb_feelist.ValidateRequired = true;
<?php } else { ?>
ftb_feelist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftb_feelist.Lists["x_barang_id"] = {"LinkField":"x_barang_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tb_barang"};

// Form object for search
var CurrentSearchForm = ftb_feelistsrch = new ew_Form("ftb_feelistsrch");
</script>
<script type="text/javascript">
...
<?php } ?>
<?php if ($tb_fee_list->TotalRecs > 0 && $tb_fee_list->ExportOptions->Visible()) { ?>
<?php $tb_fee_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($tb_fee_list->SearchOptions->Visible()) { ?>
<?php $tb_fee_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($tb_fee_list->FilterOptions->Visible()) { ?>
<?php $tb_fee_list->FilterOptions->Render("body") ?>
<?php } ?>
<?php if ($tb_fee->Export == "") { ?>
...
<?php } ?>
<?php if (($tb_fee->Export == "") || (EW_EXPORT_MASTER_RECORD && $tb_fee->Export == "print")) { ?>
<?php
if ($tb_fee_list->DbMasterFilter <> "" && $tb_fee->getCurrentMasterTable() == "tb_invoice") {
	if ($tb_fee_list->MasterRecordExists) {
?>
...
<?php } ?>
<?php
	$bSelectLimit = $tb_fee_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($tb_fee_list->TotalRecs <= 0)
			$tb_fee_list->TotalRecs = $tb_fee->SelectRecordCount();
	} else {
		if (!$tb_fee_list->Recordset && ($tb_fee_list->Recordset = $tb_fee_list->LoadRecordset()))
			$tb_fee_list->TotalRecs = $tb_fee_list->Recordset->RecordCount();
	}
	$tb_fee_list->StartRec = 1;
	if ($tb_fee_list->DisplayRecs <= 0 || ($tb_fee->Export <> "" && $tb_fee->ExportAll)) // Display all records
		$tb_fee_list->DisplayRecs = $tb_fee_list->TotalRecs;
	if (!($tb_fee->Export <> "" && $tb_fee->ExportAll))
		$tb_fee_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$tb_fee_list->Recordset = $tb_fee_list->LoadRecordset($tb_fee_list->StartRec-1, $tb_fee_list->DisplayRecs);

	// Set no record found message
	if ($tb_fee->CurrentAction == "" && $tb_fee_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$tb_fee_list->setWarningMessage(ew_DeniedMsg());
		if ($tb_fee_list->SearchWhere == "0=101")
...
<?php if ($tb_fee->Export == "" && $tb_fee->CurrentAction == "") { ?>
<form name="ftb_feelistsrch" id="ftb_feelistsrch" class="form-inline ewForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($tb_fee_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="ftb_feelistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="tb_fee">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($tb_fee_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($tb_fee_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $tb_fee_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($tb_fee_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($tb_fee_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($tb_fee_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($tb_fee_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $tb_fee_list->ShowPageHeader(); ?>
<?php
$tb_fee_list->ShowMessage();
?>
<?php if ($tb_fee_list->TotalRecs > 0 || $tb_fee->CurrentAction <> "") { ?>
<div class="panel panel-default ewGrid tb_fee">
<form name="ftb_feelist" id="ftb_feelist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tb_fee_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tb_fee_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="tb_fee">
<?php if ($tb_fee->getCurrentMasterTable() == "tb_invoice" && $tb_fee->CurrentAction <> "") { ?>
...
<?php if ($tb_fee_list->TotalRecs > 0) { ?>
<table id="tbl_tb_feelist" class="table ewTable">
...
// Header row
$tb_fee_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$tb_fee_list->RenderListOptions();
...
<?php if ($tb_fee->invoice_id->Visible) { // invoice_id ?>
	...
	<?php } else { ?>
		<th data-name="invoice_id"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tb_fee->SortUrl($tb_fee->invoice_id) ?>',1);"><div id="elh_tb_fee_invoice_id" class="tb_fee_invoice_id">
...
<?php if ($tb_fee->barang_id->Visible) { // barang_id ?>
	...
	<?php } else { ?>
		<th data-name="barang_id"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tb_fee->SortUrl($tb_fee->barang_id) ?>',1);"><div id="elh_tb_fee_barang_id" class="tb_fee_barang_id">
...
<?php if ($tb_fee->harga->Visible) { // harga ?>
	...
	<?php } else { ?>
		<th data-name="harga"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tb_fee->SortUrl($tb_fee->harga) ?>',1);"><div id="elh_tb_fee_harga" class="tb_fee_harga">
...
<?php if ($tb_fee->qty->Visible) { // qty ?>
	...
	<?php } else { ?>
		<th data-name="qty"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tb_fee->SortUrl($tb_fee->qty) ?>',1);"><div id="elh_tb_fee_qty" class="tb_fee_qty">
...
<?php if ($tb_fee->satuan->Visible) { // satuan ?>
	...
	<?php } else { ?>
		<th data-name="satuan"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tb_fee->SortUrl($tb_fee->satuan) ?>',1);"><div id="elh_tb_fee_satuan" class="tb_fee_satuan">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $tb_fee->satuan->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($tb_fee->satuan->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($tb_fee->satuan->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($tb_fee->jumlah->Visible) { // jumlah ?>
	...
	<?php } else { ?>
		<th data-name="jumlah"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tb_fee->SortUrl($tb_fee->jumlah) ?>',1);"><div id="elh_tb_fee_jumlah" class="tb_fee_jumlah">
...
<?php if ($tb_fee->keterangan->Visible) { // keterangan ?>
	...
	<?php } else { ?>
		<th data-name="keterangan"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $tb_fee->SortUrl($tb_fee->keterangan) ?>',1);"><div id="elh_tb_fee_keterangan" class="tb_fee_keterangan">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $tb_fee->keterangan->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($tb_fee->keterangan->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($tb_fee->keterangan->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
...
$tb_fee_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($tb_fee->ExportAll && $tb_fee->Export <> "") {
	$tb_fee_list->StopRec = $tb_fee_list->TotalRecs;
} else {

	// Set the last record to display
	if ($tb_fee_list->TotalRecs > $tb_fee_list->StartRec + $tb_fee_list->DisplayRecs - 1)
		$tb_fee_list->StopRec = $tb_fee_list->StartRec + $tb_fee_list->DisplayRecs - 1;
	else
		$tb_fee_list->StopRec = $tb_fee_list->TotalRecs;
}
$tb_fee_list->RecCnt = $tb_fee_list->StartRec - 1;
if ($tb_fee_list->Recordset && !$tb_fee_list->Recordset->EOF) {
	$tb_fee_list->Recordset->MoveFirst();
	$bSelectLimit = $tb_fee_list->UseSelectLimit;
	if (!$bSelectLimit && $tb_fee_list->StartRec > 1)
		$tb_fee_list->Recordset->Move($tb_fee_list->StartRec - 1);
} elseif (!$tb_fee->AllowAddDeleteRow && $tb_fee_list->StopRec == 0) {
	$tb_fee_list->StopRec = $tb_fee->GridAddRowCount;
}

// Initialize aggregate
$tb_fee->RowType = EW_ROWTYPE_AGGREGATEINIT;
$tb_fee->ResetAttrs();
$tb_fee_list->RenderRow();
while ($tb_fee_list->RecCnt < $tb_fee_list->StopRec) {
	$tb_fee_list->RecCnt++;
	if (intval($tb_fee_list->RecCnt) >= intval($tb_fee_list->StartRec)) {
		$tb_fee_list->RowCnt++;

		// Set up key count
		$tb_fee_list->KeyCount = $tb_fee_list->RowIndex;
		...
		if ($tb_fee->CurrentAction == "gridadd") {
			...
			$tb_fee_list->LoadRowValues($tb_fee_list->Recordset); // Load row values
		}
		$tb_fee->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$tb_fee->RowAttrs = array_merge($tb_fee->RowAttrs, array('data-rowindex'=>$tb_fee_list->RowCnt, 'id'=>'r' . $tb_fee_list->RowCnt . '_tb_fee', 'data-rowtype'=>$tb_fee->RowType));
...
<span id="el<?php echo $tb_fee_list->RowCnt ?>_tb_fee_barang_id" class="tb_fee_barang_id">
...
<span id="el<?php echo $tb_fee_list->RowCnt ?>_tb_fee_harga" class="tb_fee_harga">
...
<span id="el<?php echo $tb_fee_list->RowCnt ?>_tb_fee_qty" class="tb_fee_qty">
...
<span id="el<?php echo $tb_fee_list->RowCnt ?>_tb_fee_satuan" class="tb_fee_satuan">
...
<span id="el<?php echo $tb_fee_list->RowCnt ?>_tb_fee_jumlah" class="tb_fee_jumlah">
...
<span id="el<?php echo $tb_fee_list->RowCnt ?>_tb_fee_keterangan" class="tb_fee_keterangan">
...
$tb_fee_list->ListOptions->Render("body", "right", $tb_fee_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($tb_fee->CurrentAction <> "gridadd")
		$tb_fee_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($tb_fee->CurrentAction == "") { ?>
...
// Close recordset
if ($tb_fee_list->Recordset)
	$tb_fee_list->Recordset->Close();
?>
<?php if ($tb_fee->Export == "") { ?>
<div class="panel-footer ewGridLowerPanel">
<?php if ($tb_fee->CurrentAction <> "gridadd" && $tb_fee->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($tb_fee_list->Pager)) $tb_fee_list->Pager = new cPrevNextPager($tb_fee_list->StartRec, $tb_fee_list->DisplayRecs, $tb_fee_list->TotalRecs) ?>
<?php if ($tb_fee_list->Pager->RecordCount > 0 && $tb_fee_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($tb_fee_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $tb_fee_list->PageUrl() ?>start=<?php echo $tb_fee_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($tb_fee_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $tb_fee_list->PageUrl() ?>start=<?php echo $tb_fee_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $tb_fee_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($tb_fee_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $tb_fee_list->PageUrl() ?>start=<?php echo $tb_fee_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($tb_fee_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $tb_fee_list->PageUrl() ?>start=<?php echo $tb_fee_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $tb_fee_list->Pager->PageCount ?></span>
</div>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $tb_fee_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $tb_fee_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $tb_fee_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
</form>
<?php } ?>
...
<?php
	foreach ($tb_fee_list->OtherOptions as &$option)
...
<?php } ?>
<?php if ($tb_fee_list->TotalRecs == 0 && $tb_fee->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($tb_fee_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($tb_fee->Export == "") { ?>
<script type="text/javascript">
ftb_feelistsrch.FilterList = <?php echo $tb_fee_list->GetFilterList() ?>;
ftb_feelistsrch.Init();
ftb_feelist.Init();
</script>
<?php } ?>
<?php
$tb_fee_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($tb_fee->Export == "") { ?>
