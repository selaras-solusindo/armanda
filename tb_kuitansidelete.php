...
$tb_kuitansi_delete = NULL; // Initialize page object first

class ctb_kuitansi_delete extends ctb_kuitansi {
	...
	// Page object name
	var $PageObjName = 'tb_kuitansi_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("tb_kuitansilist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in tb_kuitansi class, tb_kuitansiinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("tb_kuitansilist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($tb_kuitansi_delete)) $tb_kuitansi_delete = new ctb_kuitansi_delete();

// Page init
$tb_kuitansi_delete->Page_Init();

// Page main
$tb_kuitansi_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tb_kuitansi_delete->Page_Render();
?>
...
var CurrentForm = ftb_kuitansidelete = new ew_Form("ftb_kuitansidelete", "delete");

// Form_CustomValidate event
ftb_kuitansidelete.Form_CustomValidate = 
...
ftb_kuitansidelete.ValidateRequired = true;
<?php } else { ?>
ftb_kuitansidelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftb_kuitansidelete.Lists["x_invoice_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_no_invoice","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tb_invoice"};
...
<?php $tb_kuitansi_delete->ShowPageHeader(); ?>
<?php
$tb_kuitansi_delete->ShowMessage();
?>
<form name="ftb_kuitansidelete" id="ftb_kuitansidelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tb_kuitansi_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tb_kuitansi_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="tb_kuitansi">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($tb_kuitansi_delete->RecKeys as $key) { ?>
...
<?php if ($tb_kuitansi->no_kuitansi->Visible) { // no_kuitansi ?>
		<th><span id="elh_tb_kuitansi_no_kuitansi" class="tb_kuitansi_no_kuitansi"><?php echo $tb_kuitansi->no_kuitansi->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$tb_kuitansi_delete->RecCnt = 0;
$i = 0;
while (!$tb_kuitansi_delete->Recordset->EOF) {
	$tb_kuitansi_delete->RecCnt++;
	$tb_kuitansi_delete->RowCnt++;

	// Set row properties
	$tb_kuitansi->ResetAttrs();
	$tb_kuitansi->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$tb_kuitansi_delete->LoadRowValues($tb_kuitansi_delete->Recordset);
...
<?php } ?>
<?php if ($tb_kuitansi->no_kuitansi->Visible) { // no_kuitansi ?>
		<td<?php echo $tb_kuitansi->no_kuitansi->CellAttributes() ?>>
<span id="el<?php echo $tb_kuitansi_delete->RowCnt ?>_tb_kuitansi_no_kuitansi" class="tb_kuitansi_no_kuitansi">
...
<?php
	$tb_kuitansi_delete->Recordset->MoveNext();
}
$tb_kuitansi_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tb_kuitansi_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ftb_kuitansidelete.Init();
</script>
<?php
$tb_kuitansi_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tb_kuitansi_delete->Page_Terminate();
?>
