...
class ctb_invoice_view extends ctb_invoice {
	...
	function SetupOtherOptions() {
		...
		$links = "";
		if ($GLOBALS["tb_fee_grid"] && $GLOBALS["tb_fee_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'tb_fee')) {
		...
		if ($GLOBALS["tb_fee_grid"] && $GLOBALS["tb_fee_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'tb_fee')) {
		...
		if ($GLOBALS["tb_fee_grid"] && $GLOBALS["tb_fee_grid"]->DetailAdd && $Security->CanAdd() && $Security->AllowAdd(CurrentProjectID() . 'tb_fee')) {
		...
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'tb_fee');
		if ($item->Visible) {
	...
	function ExportData() {
		...
		// Export detail records (tb_fee)
		if (EW_EXPORT_DETAIL_RECORDS && in_array("tb_fee", explode(",", $this->getCurrentDetailTable()))) {
			global $tb_fee;
			if (!isset($tb_fee)) $tb_fee = new ctb_fee;
			$rsdetail = $tb_fee->LoadRs($tb_fee->GetDetailFilter()); // Load detail records
			if ($rsdetail && !$rsdetail->EOF) {
				...
				if ($this->Export <> "csv" || EW_EXPORT_DETAIL_RECORDS_FOR_CSV) {
					...
					$tb_fee->ExportDocument($Doc, $rsdetail, 1, $detailcnt);
				}
...
<?php if ($tb_invoice->terbayar->Visible) { // terbayar ?>
	<tr id="r_terbayar">
		<td><span id="elh_tb_invoice_terbayar"><?php echo $tb_invoice->terbayar->FldCaption() ?></span></td>
