...
class crGantt extends crChart {
	...
	function SetupStartEnd() {
		...
		$sql = "SELECT MIN(" . ewr_QuotedName($this->TaskStartField, $dbid) . "), MAX(". ewr_QuotedName($this->TaskEndField, $dbid) . ") FROM " . ewr_QuotedName($this->TaskTable, $dbid);
...
//			} elseif ($interval == EWR_GANTT_INTERVAL_MONTH) {
	...
	function OutputDataTable() {
		...
		$conn = &$this->Table->Connection();
		$dbid = $this->Table->DBID;
		$sql = "SELECT * FROM " . ewr_QuotedName($this->ProcessTable, $dbid);
		$rs = $conn->Execute($sql);
		if ($rs && !$rs->EOF) {
			$i = 0;
	...
	function OutputTasks() {
		...
		$conn = &$this->Table->Connection();
		$sql = "SELECT * FROM " . ewr_QuotedName($this->TaskTable, $this->Table->DBID);
		if ($this->TaskFilter <> "") $sql .= " WHERE " . $this->TaskFilter;
	...
	function OutputProcesses() {
		...
		if ($this->ProcessTable <> "") { // Use process table
			...
			$sql = "SELECT DISTINCT " . ewr_QuotedName($fid, $dbid) . " AS id, " .
				ewr_QuotedName($fname, $dbid) . " AS name, " . ewr_QuotedName($fstart, $dbid) . " FROM " . ewr_QuotedName($this->TaskTable, $dbid);
	...
	function Xml() {
		...
		if ($this->MilestoneTable <> "") {
			$sql = "SELECT * FROM " . ewr_QuotedName($this->MilestoneTable, $dbid);
			...
			$sql = "SELECT " . ewr_QuotedName($this->TaskIdField, $dbid) . " AS ". ewr_QuotedName('taskId', $dbid) . ", " .
				...
				" FROM " . ewr_QuotedName($this->TaskTable, $dbid) .
		...
		if ($this->TrendlineTable <> "") {
			$sql = "SELECT * FROM " . ewr_QuotedName($this->TrendlineTable, $dbid);
		...
		if ($this->ConnectorTable <> "") {
			$sql = "SELECT * FROM " . ewr_QuotedName($this->ConnectorTable, $dbid);
			...
			$sql = "SELECT " . ewr_QuotedName($this->TaskFromTaskIdField, $dbid) . " AS ". ewr_QuotedName('fromTaskId', $dbid) . ", " .
				ewr_QuotedName($this->TaskIdField, $dbid) . " AS ". ewr_QuotedName('toTaskId', $dbid) . " FROM " . ewr_QuotedName($this->TaskTable, $dbid);
			$this->OutputQuery($sql, 'connectors', 'connector', $this->ConnectorsAttrs, $this->ConnectorAttrs);
		}
